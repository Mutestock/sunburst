...
use scraper::{ handlers::article_handler::article_insert_many, utils::config::{MONGO_CONN_STRING, CONFIG},
};
...
async fn test_article_insert_many() -> Result<(), mongodb::error::Error> {
    set_testing_mode();
    let db = connect_mongo(&MONGO_CONN_STRING).await.unwrap().default_database().unwrap();
